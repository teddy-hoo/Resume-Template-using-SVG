...
Template.prototype._addStyle = function(element, styleName, styleValue){
	re = new RegExp(styleName + "\\([0-9]*[a-z]*\\)?", "g");
...
Template.prototype.addTitle = function(title){
	...
	titleRect.setAttribute("d", "m " + this.leftIndent + " 0 " +
	...
	titleText.setAttribute("x", window.innerWidth / 2);
	titleText.setAttribute("y", this.titleHight / 2);
...
Template.prototype.addEventList = function(eventList){
	...
	eventListG.appendChild(eventItems);
...
Template.prototype._createEventItem = function(eventItem){
	var that = this;
	var eventItemG = document.createElementNS(SVGNS, "g");
	eventItem.forEach(function(item){
		...
		eventItemG.appendChild(itemTitle);
	});
	return eventItemG;
};
...
Template.prototype._fixLayout = function(element){
	...
	for(var i = 0; i < children.length; ++i){
		...
		height += parseInt(children[i].getBBox().height);
